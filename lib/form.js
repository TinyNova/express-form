...
function form() {
    const routines = Array.prototype.slice.call(arguments);
    const options = form._options;
    ...
    return function (req, res, next) {
        ...
        if (options.passThrough) {
            req.form = utils.clone(mergedSource);
        }
        ...
        async.each(routines, function(routine, cb) {
            routine.run(mergedSource, req, options, function(err, result) {
                // return early if no errors
                if (!Array.isArray(result) || !result.length) {
                    return cb(null);
                }

                const errors = req.form.errors = req.form.errors || [];
